...
class FrameFormation {
	...
	public Image[] convertToFrames(String url){
		...
		try{
			...
			ShowProcess1.println("fpc"+fPC);
			ShowProcess1.println("fGc"+fGC);
			while(i == (seek = fPC.seek(i))){
				...
				FrameToImage(image2,"anomaly\\SplitedFrames\\VBPro"+i+".jpeg");
				al.add(image2);
				if(i++ == 200)break;
			}
...
//			img = (NamedImage[]) al.toArray();
			...
			ShowProcess1.println("image length: "+img.length);
			}catch(Exception e){
				ShowProcess1.println("Exception "+ e); 
			}
			System.gc();
	...
	public Image nextFrame(){
		...
		FrameToImage(image2,"anomaly\\SplitedFrames\\VBPro"+framePosition+".jpeg");
		try{
		TrainingView.jTextFieldDetectedFrameName1.setText("anomaly\\SplitedFrames\\VBPro"+framePosition+".jpeg");
		}
		catch (NullPointerException e) {
			// TODO: handle exception
		}
		try{
		AnomalyDetectionView.jTextFieldDetectedFrameName.setText("anomaly\\SplitedFrames\\VBPro"+framePosition+".jpeg");
		}
		catch (NullPointerException e) {
			// TODO: handle exception
		}
	...
	public String getMediaPath(){
		...
		ShowProcess1.println("path = "+path); 
		return "file:"+path;
	}
	...
	private void FrameToImage(Image image,String path){
		...
		{
			ShowProcess1.println("Jpeg Writer"+ e);
		}
	...
	public int saveImageObj(Image[] image){
		...
		for(i=0; i < image.length; i++){
				...
				String path="anomaly\\SplitedFrames\\VBPro"+i+".jpeg";
